...
$latin=
	"aabvgdeejcziyklmnnooprstuufxhscssъieua";
...
//	return $k2 * 256 + $k1;
//}
function unichr($intval){//http://pastebin.com/fmiSnNin
return mb_convert_encoding(pack('n', $intval) , 'UTF-8', 'UTF-16BE');
}
for($i=0x410/*А*/;$i<=0x44f/*я*/;$i++){
define(unichr($i),u(unichr($i)));
}
$t='әөүҗңһӘӨҮҖҢҺёЁəƏɵƟşŞñÑ';
$t=preg_split('//u',$t);
for($i=1;$i<=count($t)-1;$i++){
define($t[$i],u($t[$i]));
}
for($i=0x41/*A*/;$i<=0x7a/*z*/;$i++){
define(unichr($i),u(unichr($i)));
}
define('APSTRPH',u('\''));
...
$convertyeyuya=array(
е=>KICI_Y.KICI_Ы,ю=>KICI_Y.u,я=>KICI_Y.a,й=>KICI_Y,
Е=>ZUR_Y.KICI_Ы,Ю=>ZUR_Y.u,Я=>ZUR_Y.a,Й=>ZUR_Y,
);
$convertrus=array(
...
о=>o,п=>p,р=>r,с=>s,т=>t,
ф=>f,х=>x,
ч=>KICI_C,ш=>ş,щ=>ş.KICI_C,ъ=>'',ы=>KICI_Ы,ь=>APSTRPH,э=>e,
А=>A,Б=>ZUR_B,В=>V,Г=>G,Д=>D,
Ж=>ZUR_Ж,З=>Z,И=>ZUR_I,Й=>ZUR_Y,К=>K,Л=>L,М=>M,Н=>N,
О=>O,П=>P,Р=>R,С=>S,Т=>T,
Ф=>F,Х=>X,
Ч=>ZUR_C,Ш=>Ş,Щ=>Ş.ZUR_C,Ъ=>'',Ы=>ZUR_Ы,Ь=>APSTRPH,Э=>E,
);
$basqarussuz=array(
u('аватар'),u('рия'),u('авиа'),u('кукмара'),u('кучук'),u('чигайка'),u('вамин'),u('магазин'),u('реклама'),u('кама'),
u('камал'),u('тдгпу'),u('газет'),u('курган'),u('ульян'),u('тнв'),u('алфавит'),u('куласа'),u('ерэ'),u('код'),u('кабинет'),
u('конкурс'),u('егэ'),u('диктант'),u('камил'),u('ваз'),u('газ'),u('музыка'),u('кандидат'),u('округ'),u('арканзас'),
u('канзас'),u('португал'),u('канада'),u('украин'),u('карта'),u('поезд'),u('нияз'),u('казбек'),u('заказ'),u('указ'),
u('балкан'),u('команда'),u('лига'),
);
...
Е=>ZUR_Y.E,Ю=>ZUR_Y.ZUR_U,Я=>ZUR_Y.Ə,Й=>ZUR_Y,
);
function zur($s){
...
u('карават')=>u('karawat'),u('мәгариф')=>u('mə').KICI_G.u('ərif'),u('гатаулл')=>KICI_G.u('ataull'),
u('пауза')=>u('pauza'),u('мөгаен')=>u('mɵ').KICI_G.u('ayen'),u('мөлаем')=>u('mɵlayem'),
u('биектау')=>u('biyektaw'),u('төбәкара')=>u('tɵbəkara'),u('акъегет')=>u('aqyeget'),
);
$iskarmatatarsuzconvert=array(//махсус ысул белән әйләндерү һәм урыс сүзе итеп тамгаланса ул тамганы алып ата
u('акш')=>u('aqş'),u('юхиди')=>KICI_Y.u('xidi'),u('тяг')=>u('ty').KICI_G,
u('ядкяр')=>KICI_Y.u('adkar'),u('никях')=>u('nikax'),u('аккош')=>u('aqqoş'),
);
$iskarmatatarsuz=array(//урыс сүзе тамгасы табылганнан соң тикшереп кире татар сүзенә әйләндерү
u('солтан'),//солтангалиев дигәндә
u('веб'),u('гомум'),u('коточкыч'),u('гомер'),u('гореф'),u('гошер'),//(монда KICI_G язасы түгел)
);
/*
function aylandir($s){
$ss=explode( ,$s);
...
$zuraytyeyoyuya,$iskarmaconvert,$iskarmatatarsuzconvert,$debug,$iskarmatatarsuz,$bd
	;
	$s=mb_convert_encoding($s,'ucs2','utf8');
					...
					if(!$russuzdir&&!$russuziyuq){
						if($hij==о||$hij==О){
							...
							if($j==4&&($h==б||$h==Б)){//башкорт
								...
								if(
mb_strpos($birsuzbaso,о.в)!==false||mb_strpos($birsuzbaso,в.о)!==false//беренче 3 хәрефтә ов булса йәки во булса
								){
									$russuzdir=true;
								}else{
								}
...
zur($russuzhp1)||!kirillitsa($russuzhp1)
				...
				while($i2<$birsuzo){
					...
					if($birsuzh==г||$birsuzh==Г){
						//include('ttcyttla-g.php');
//ГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГГ
						//$ns.=ok;
						//if($suzbaso){
						//	for($j=0;$j<$guto;$j++){
...
$birsuzh==е||$birsuzh==ю||$birsuzh==я||$birsuzh==Е||$birsuzh==Ю||$birsuzh==Я
						...
						//include('ttcyttla-yeyuya.php');
//ЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮЯЕЮ
...
zur($birsuzhp1)||!kirillitsa($birsuzhp1)
								...
								$ns.=($birsuzh==ё?APSTRPH.o:APSTRPH.O);
							}
						}
					}elseif($birsuzh==к||$birsuzh==К){//
						//include('ttcyttla-k.php');
//КККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККККК
						//$ns.=OK;
						//if($suzbaso){
						//	for($j=0;$j<$kuto;$j++){
...
zur($birsuzhp3)||!kirillitsa($birsuzhp3)
								...
								$i2+=2;
							//}elseif($birsuzhp2==й||$birsuzhp2==Й){
							}else{
								$j=1;
								do{
									$j+=1;
...
$birsuzhm1==ә||$birsuzhm1==а||$birsuzhm1==Ә||$birsuzhm1==А//маэмай,тәэсир һәм әмма тәэссорат, ризаэтдин
								...
								if(tartoq($birsuzhp1)&&tartoq($birsuzhp2)){
									$ns.=($birsuzh==э?APSTRPH.e:APSTRPH.E);
								}else{
									$ns.=APSTRPH;
								}
							}else{
...
$birsuzhm1==о||$birsuzhm1==е||$birsuzhm1==О||$birsuzhm1==Е
	...
	//return $ns;
	return mb_convert_encoding($ns,'utf8','ucs2');
}

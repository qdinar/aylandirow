...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat( $text ) {
		...
		if ( preg_match( '/^(җигәнтама|йөрәктау|тимер(булат|чыбык)|кайсыбер)/u', $text )
		) {
			$parts = preg_split( '/(?<=^.....)/u', $text, null, PREG_SPLIT_NO_EMPTY );
			return $this->convertCompoundTatarWordCyrlToLat( $parts );
		}
		...
		if ( preg_match( '/(?<=.)(?<!сосн|град)(?<!самин|датир)(?<!петропавл)'
			. '(?<!покр)(?<!^новоникола)(?<!^кодир)'
			// (?<!^балабан)(?<!^белоус)
		...
		if (
			...
			|| preg_match( '/^(к('
				. 'а(н(дидат|а(л|да|ш)|түк)|бина|м(аз|зул|би)'
					...
					. '|у(чук|ф)|за(нка|да|ки)|ли(й|гула)|питал'
					. '|лу|спий|чал|кур|йман)'
				// do not catch карлар
				...
				// . 'ама(?!лыш)'
				. '|у(р(с|асан|тка)|к(мара|уруза)|туз|зьм|льт|бан|пала|ба)'
				. '|о(м(и(сс|$)|анд(а|ир)|мун|б(инат|айн))'
					. '|н(с(тан|ул)|ка)'
					. '|д($|ын?|та(н|гы)?|ка(ча)?|сыз|чы|ның|иров|лы)'
					. '|жан|фта|с(ынка|та)|р(аб|пус)|лум|ка|ала|х|лбурн)'
				. '|изнер'
				. '))/u', $text )
			...
			|| preg_match(
				...
				. '|а(ук|на|м(аш|бия)|зимов|р(ант|сия|ри)'
				. '|л(мудуг|лия)|биния|й$|рхипел|ити)'
				. '|(о(р(илла|ки|ьк)|а|би|лланд))'
				. '|вин|у(м(анитар|ми)|бан)|еве)/u',
				$text )
			...
			|| preg_match(
				'/^(б(а(гира|лабан|бынин|вар|рнаул|нк|йт|гама)'
					. '|о(л(гария|ив(ия|ар))|тинка|гдан)'
					. '|елоус|ензин|исау|ушу|урка)'
					. '|та(гил|миянг)|х(орватия|ими|арьк)|маугли|даут|юрист'
					. '|юбиле|л(и(га|тва)|атвия|окк)|система|республика'
					. '|м(а(гнит|ксим|ргарита)|олдавия)|органик|извест|нефть'
					. '|и(юн|жау|нка|н(дивид|тер))|дев|самин|эфир|алфавит|васили|савин'
					. '|нижгар|пермь|адмирал|дискусс|эласт|сингапур|физи|агалега'
					. '|окру|дивизи|субъект|объект|жук|юхн|рабфа|фукс)'
				. '/u', $text )
			...
			|| preg_match(
				'/^(м(у(зыка|скул)|е(лвин|ркель)|а(даг|р(шан|ка)|йка))'
					. '|в(епс|ышка|а(лда|ссал)|уз|акуум)'
					. '|п(а(уза|рагвай)|о(дкаст|вар))|(за|у)каз'
					. '|с(о(с(тав|нов)|к(рат)?|г)|а(рат|ван|па)|ьерра|ервер)'
					. '|энерг|юлия|фея|яков|порту'
					. '|лив(?!ә)|уганда|елена|тиран'
					. '|о(скар|трад|кт)'
					. '|д(екрет|углас|арья|иалект)|июл|н(аум|ерв)|талала'
					. '|ре(жисс|зерв)|фуфай'
					. '|ла(всан|йкра|нка)|пояс|йошкар|ноя|датир|чабаксар?'
					. '|а(ксаков|грар|угуст|рхи|ньелли)'
					. '|т(анк|о[кг])|юридик|янка|сервис|зигзаг)'
					. '|х(авьер|орват)|уаскаран|марк|эпикур|фолликул|йорк'
	...
	function processWordWithRussianStemCyrlToLat( $text ) {
		...
		$parts = preg_split(
			'/((?<!и|^ш)(?<!^(ёл|де|ян|ко|ин))'
			. '(?<!^(шпа|щёт|вол|выш|май|бур|кеп|кон|оме|бан|лан|мар))(?<!нош)'
		...
		$parts = preg_split(
			'/(?<!аматур)(?<!о)(?<!^ма)(?<!^хок|бан|мер)(?<!^(физи|окру|нинс))'
			.'(?=[гк][ые]($|[^нтр]))(?!гель)/u',
			$text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
			return $this->convertRussianWordWithTatarEndingCyrlToLat( $parts );
		...
		$parts = preg_split( '/(?<!^.)(?<!леку|ярос|нико|югос|купа|авос|умми)'
			. '(?<!вац|обк|шка|дуг|коа)(?<!^(ве|га))(?<!^архипе)'
			. '(?=л[аә])'
			. '(?!ла(зма|кир|бан))/u',
			$text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
	...
	function convertTatarWordWithArabicRootFromCyrillicToLatin( $text ) {
		...
		$text = preg_replace( '/(?<=^әгъз)а/u', 'ä', $text );
	...
	function convertLowercasedTatarWordLatToCyrl( $text ) {
		...
		*/
		...
		// (cons except yln)ts by default is (cons except yln)ц
		$text = preg_replace( '/(?<![aouıiäöüeаяюеауүэыyln])ts/u', 'ц', $text );
		// exception to following rule
		...
		*/
		...
		$text = preg_replace( '/(?<!(jabl|atel|vest|neft))\''
			.'(?=n[eı]ñ?|t[aä]n?|d[aä]n?|ğa|gä|qа|kä|l(ıq?|ek?))/u', '-', $text );
		...
		$text = preg_replace( '/(?<=s)ü(?=rä)/u', 'у', $text );
		// e
					...
					if(
						...
						&& (
							preg_match(
								...
								.'n[eı](ñ|[ğq]ı|[gk]e)?$'
								.'|(t[aä]|d[aä])([ğq]ı|[gk]e)?$'
								.'|(t[aä]n|d[aä]n|ğa|gä|qа|kä)(ç[aä])?$'
								.'|l(ıq?|ek?)'
								.')/u',
								$words[$i]
							)
